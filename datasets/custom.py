...
class CustomAlignedDataset(Dataset):
    def __init__(self, dataset_config, stage='train'):
        ...
        self.imgs_ori = ImagePathDataset(image_paths_ori, self.image_size, flip=self.flip, to_normal=self.to_normal, channels=dataset_config.channels)
        self.imgs_cond = ImagePathDataset(image_paths_cond, self.image_size, flip=self.flip, to_normal=self.to_normal, channels=dataset_config.channels)
    def __len__(self):
...
class CustomInpaintingDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #     image = image.convert('RGB')
        if self.channels == 1:
            image = image.convert('L')  # 'L' là chế độ Grayscale (1 kênh)
